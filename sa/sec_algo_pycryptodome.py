...
ErrorCodes = (NO_ALG, NO_MODE)

PAD_MODES = {'cbc'}
IV_MODES = {'cbc', 'cfb', 'ofb'}
NONCE_MODES = {}
...
def keygen_shared(key_size, alg, mode, key_mat = None):
    if key_mat == None:
        new_key =  get_random_bytes(key_size // 8)
    else:
        new_key = key_mat
    key_dict = {'alg' : alg,
                'size': key_size,
...
def keygen_public(key_size, alg, hash_alg, curve):
    if alg == 'rsa':
        ...
        if curve == 'p256': # FIPS 186-4, Section D.1.2.3
            key_pair = ECC.generate(curve = 'P-256')
            priv_key = key_pair.export_key(format = 'PEM')
            pub_key = key_pair.public_key().export_key(format = 'PEM')
            ...
            print('SA_ERROR:', curve, 'not available.', flush = True)
    else:
