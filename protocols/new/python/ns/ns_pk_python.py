...
class NS_Client():
    ...
    def ns_initiate(self, b_host, b_port):
        ...
        #Send M3
        self.c_socket.sendto(b'm3' + cipher_B.encrypt(
            pickle.dumps([nonce_a, ('A', self.c_host, self.c_port)])), (b_host, b_port))
        #Receive M6
        ...
        print('M6:', m6)
        if m6[0] != nonce_a:
            print('NS_Client: Protocol Failure: Initiator nonce returned',
                  'by remote client does not match.')
            return
        #Send M7
...
class NS_Recv():
    ...
    def receive(self):
        ...
        print('M3:', m3)
        #send M4
        self.socket_b.sendto(b'm4' + pickle.dumps([('B', self.host_b, self.port_b), m3[1]]),
                             (self.host_as, self.port_as))
        #Receive M5
        ...
        if not verifier.verify(SHA256.new(pickle.dumps([m5[0], m5[1]])), m5[2]):
            print('NS_Recv: Protocol Failure: Could not verify signature on',
                  'message from authentication server.')
            return
        if m5[1] != m3[1]:
            print('NS_Recv: Protocol Failure: Intiator identity returned by',
                  'authentication server does not match.')
            return
        pk_A = RSA.importKey(m5[0])
        nonce_b = getrandbits(NONCE_SIZE)
        cipher_A = PKCS1_OAEP.new(pk_A)
        #Send M6
        self.socket_b.sendto(b'm6' + cipher_A.encrypt(pickle.dumps([m3[0], nonce_b])),
                             (m3[1][1], m3[1][2]))
        #Receive M7
        m7_raw = self.socket_b.recv(4096)
        #Check tag on message
        if m7_raw[:2] != b'm7':
        ...
        print('M7:', m7)
        if m7 != nonce_b:
            print('NS_Recv: Protocol Failure: Initiator nonce returned',
                  'by remote client does not match.')
            return
...
if __name__ == '__main__':
    ...
    ns_as = NS_AS(AS_HOST, AS_PORT, SK_AS, ('A', A_HOST, A_PORT),
                  ('B', B_HOST, B_PORT), SK_A.publickey(), SK_B.publickey())
    ns_as.begin()
