...
import sa.timers as SAT
full_path = '/home/christopher/secalgo/protocols/new/python/ns/'
result_path = '/home/christopher/secalgo/protocols/new/python/ns/results/'
raw_path = '/home/christopher/secalgo/protocols/new/python/ns/rawData/'
...
def time_exp02(p, iter_num, iter_label):
    if p in protocols:
        loops = SAT.proto_loops['__main__']
        print('Protocol Timing Experiment for:', p, '-', loops, flush = True)
        ...
        for i in the_range:
            ...
            f_txt = open(raw_path + p + '_' + str(loops) + '_' +
                         str(i + 1) + results_ext, 'w')
            f_err = open(raw_path + p + '_' + str(loops) + '_' +
                         str(i + 1) + error_ext, 'w')
        ...
        print('Completed Timing Experiment for:', p, '-', loops, flush = True)
    else:
...
def parse_exp02(p, iter_num, iter_label, output_file):
    loops = SAT.proto_loops['__main__']
    if output_file == None:
        of = open(result_path + p + '_' + str(loops) + '_output.txt', 'w')
    else:
    ...
    if p in protocols:
        ...
        for i in range(iter_count):
            protocol_time = 0
            with open(raw_path + p + '_' + str(loops) + '_' +
