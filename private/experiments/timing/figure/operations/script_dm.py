...
def time_SA_sym_keygen(loops, testsize):
    ...
    for i in range(loops):
        ...
        result = SA.keygen('shared', keysize = (testsize * 8))

    end_wc = time.perf_counter()
    end_cpu = resource.getrusage(resource.RUSAGE_SELF)

    return collect_raw('SA_sym_keygen', test_size,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_sym_encrypt()

def time_PyCrypto_sym_keygen(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)

    for i in range(loops):
        from Crypto import Random
        new_key = Random.new().read(testsize)
        result = {'alg' : 'AES',
                  'size' : 256,
                  'mode' : 'CBC',
                  'key' : new_key}
    ...
    return collect_raw('PC_sym_keygen', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_sym_keygen()    

def time_SA_sym_encrypt(loops, testsize):
    data = TEST_DATA[:testsize]
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)

    for i in range(loops):
        import sa.secalgo as SA
        result = SA.encrypt(data, key = TEST_SYM_KEY)

    end_wc = time.perf_counter()
    end_cpu = resource.getrusage(resource.RUSAGE_SELF)

    TEST_SYM_CT = SA.encrypt(data, key = TEST_SYM_KEY)
                           
    return collect_raw('SA_sym_encrypt', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_sym_encrypt()
                           
def time_PyCrypto_sym_encrypt(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    for i in range(loops):
        ...
        serial_data = pickle.dumps(TEST_DATA[:testsize])
    ...
    return collect_raw('PC_sym_encrypt', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_sym_encrypt()

def time_SA_sym_decrypt(loops, testsize): 
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    assert result == TEST_DATA[:testsize]
    
    return collect_raw('SA_sym_decrypt', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_sym_decrypt()

def time_PyCrypto_sym_decrypt(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    return collect_raw('PC_sym_decrypt', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_sym_decrypt()

def time_SA_mac_sign(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    
    for i in range(loops):
        import sa.secalgo as SA
        result = SA.sign(TEST_DATA[:testsize], key = TEST_MAC_KEY)

    end_wc = time.perf_counter()
    end_cpu = resource.getrusage(resource.RUSAGE_SELF)

    TEST_MAC = SA.sign(TEST_DATA[:testsize], key = TEST_MAC_KEY)
    
    return collect_raw('SA_mac_sign', testsize
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_mac_sign()

def time_PyCrypto_mac_sign(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    return collect_raw('PC_mac_sign', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_mac_sign()

def time_SA_mac_verify(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    return collect_raw('SA_mac_verify', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_mac_verify()

def time_PyCrypto_mac_verify(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    assert data == TEST_DATA[:testsize]
    
    return collect_raw('PC_mac_verify', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_mac_verify()

def time_SA_pub_keygen(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)

    for i in range(loops):
        import sa.secalgo as SA
        result = SA.keygen('public', keysize = (testsize * 8))
        
    end_wc = time.perf_counter()
    end_cpu = resource.getrusage(resource.RUSAGE_SELF)
    
    return collect_raw('SA_pub_keygen', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_SA_pub_keygen()

def time_PyCrypto_pub_keygen(loops, testsize):
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)

    for i in range(loops):
        from Crypto.PublicKey import RSA
        key_pair = RSA.generate(testsize * 8)
    ...
    return collect_raw('PC_pub_keygen', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
#end def time_PyCrypto_pub_keygen()

def time_SA_pub_encrypt(loops, testsize):
    data = TEST_DATA[:testsize]
    start_wc = time.perf_counter()
    start_cpu = resource.getrusage(resource.RUSAGE_SELF)
    ...
    for i in range(loops):
        import sa.secalgo as SA
        result = SA.encrypt(data, key = TEST_PUB_KEY)

    end_wc = time.perf_counter()
    end_cpu = resource.getrusage(resource.RUSAGE_SELF)

    return collect_raw('SA_pub_encrypt', testsize,
                       (start_wc, start_cpu),
                       (end_wc, end_cpu),
                       loops)
